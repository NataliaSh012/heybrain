...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function checkSetting() {
    chrome.storage.local.get(["enabled"]).then((result) => {
      ...
      switchButton.checked ? continueWork() : stopWork();
    });
  }

  function stopWork() {
  
    signOut.style.background = "pink";
    // chrome.storage.local.get(["access_token", "user"], (data) => {

    //   const tempStorage = {accessToken: data.access_token};
    //   chrome.storage.local.set(tempStorage).then(() => {
    //     console.log("temporaly stored:", tempStorage);
    //   });
  ...
  function continueWork() {
    ...
    signOut.style.background = "lightblue";
